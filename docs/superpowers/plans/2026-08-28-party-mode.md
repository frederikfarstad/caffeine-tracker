...
- **Team name is `Fleks`** — that exact spelling, capital F, everywhere. Never "Flex", never "Ovio", never "Teoria".
...
```bash
grep -rn "Ovio\|Teoria" --include="*.ts" --include="*.tsx" --include="*.md" src README.md SETUP.md
```
...
`SETUP.md` line 224: replace "but if Ovio or Teoria treats this as a company tool" with "but if Fleks treats this as a company tool".
